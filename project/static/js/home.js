...
function fetchSensorIDs(){
    ...
    fetch(`/sensors_ids/${typeid}`).then(response => response.json()).then(data => {
...
function fetchData(){
    ...
    .then(response => response.json()).then(data => 
        {if (data){
                ...
                if (avgData){
                    Array.from(document.getElementsByClassName('no2Value')).forEach(element => {
                        element.textContent = avgData.no2;
                    });
                    
                    Array.from(document.getElementsByClassName('pm2_5Value')).forEach(element => {
                        element.textContent = avgData.pm2_5;
                    });
                    
                    Array.from(document.getElementsByClassName('pm10Value')).forEach(element => {
                        element.textContent = avgData.pm10;
                    });
                }   
                // console.log(data);
                if (data.last_updated){
                    let lastUpdated = new Date(`${data.last_updated['obs_date']} ${data.last_updated['obs_time_utc']}`);
                    document.getElementById('lastUpdated').textContent = relativeTime(lastUpdated);
...
function relativeTime(date) {
  ...
  if (interval > 1) {
    return Math.floor(interval) + " years ago (" + date.toLocaleDateString() + ")";
  }
  interval = seconds / 2592000;
  if (interval > 1) {
    return Math.floor(interval) + " months ago (" + date.toLocaleDateString() + ")";
  }
...
function newLineChartObj(chartId){
...
for (let param of parameters){
    newLineChartObj(`${param.toLowerCase()}LineChart`);
}
...
function updateLineCharts(data){
    ...
    if (data){
        ...
        for (let param of parameters){
            let chartObj = document.getElementById(`${param.toLowerCase()}LineChart`).chartInstance;
            chartObj.data.datasets[0].label = `Minutely ${param.toUpperCase()}` //y-axis label
