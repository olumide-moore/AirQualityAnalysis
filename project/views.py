...
def get_sensor_data(request, sensor_type, sensor_id, start, end):
    ...
    if sensor_id:
        ...
        minutely_avgs = sensor.getMinutelyAverages(start_datetime, end_datetime)
        minutely_avgs_dict = convert_df_to_dict(minutely_avgs)
        hourly_avgs = sensor.getHourlyAverages(start_datetime, end_datetime)
        hourly_avgs_dict = convert_df_to_dict(hourly_avgs)
        last_updated = sensor.getLastUpdatedTime()
        # print(last_updated)

        no2_hourly_avgs= sensor.getNO2_HourlyAvgs(minute_threshold=45)
        no2_hourly_avg_max = no2_hourly_avgs.max()
        if no2_hourly_avg_max in [np.nan, None]:
            no2_hourly_avg_max = None
        pm2_5_24houravg = sensor.getPM2_5_24HourAvg()
        pm10_24houravg = sensor.getPM10_24HourAvg()

        hourly_aqis = aqi_obj.compute_hourly_aqis(hourly_avgs_dict)
        aqi_data = aqi_obj.compute_aqi(no2_hourly_avg_max, pm2_5_24houravg, pm10_24houravg)
