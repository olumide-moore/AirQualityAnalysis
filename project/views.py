...
# #49, 47,29, 11
# start= time.time()
# sensor_type_and_id_fetcher = SensorTypeAndIdFetcher()
# print('Time taken to fetch all data: ', (time.time()-start) * 1000, 'ms') 
# data_fetcher= SensorDataFetcher(requiredConcentrations=['no2', 'particulatepm10', 'particulatepm2_5'])
# aqi_obj = AQI()
...
#                             'date': date})
# def get_sensor_types():
#     return sensor_type_and_id_fetcher.getSensorTypes()

# def get_sensor_ids(request, type_id):
#     start= time.time()
#     sensorIds= sensor_type_and_id_fetcher.getSensorIdsOfTypeId(type_id)
#     print('Time taken: ', (time.time()-start) * 1000, 'ms')
#     return JsonResponse(
#         {'sensors': sensorIds}
#     )
