...
class SensorDataFetcher():
    ...
    def set_sensor_type(self, sensor_type : str):
        ...
        self.set_sensor_table(sensor_type)

    def get_sensor_type(self) -> str:
        """
        returns the sensor_type
        """
        return self.sensor_type

    def set_sensor_table(self, sensor_type : str):
        """
        Sets the sensor_table based on the sensor_type

        :param sensor_type: str
        """
        if sensor_type == 'Zephyr':
            self.sensor_table= AllSensorMeasurementsWithLocationsZephyr
        elif sensor_type == 'SensorCommunity':
            self.sensor_table= AllSensorMeasurementsWithLocationsSc
        elif sensor_type == 'Plume':
            self.sensor_table= AllPlumeMeasurements
        else:
            self.sensor_table= None
    ...
    def fetch_last_updated(self) -> str:
        ...
        """
        if self.sensor_table is None or self.sensor_id is None:
            return None
        if self.last_updated is not None: 
            return self.last_updated #return the last updated date if it is already fetched
        try:
            last_updated_record = self.sensor_table.objects.values('obs_date','obs_time_utc').filter(sensor_id=self.sensor_id).latest('obs_date', 'obs_time_utc')
            if last_updated_record.get('obs_date') and last_updated_record.get('obs_time_utc'):
                ...
                self.last_updated = None
        except self.sensor_table.DoesNotExist:
            self.last_updated = None
    ...
    def update_cache(self, rawdata : dict, hourly_avgs : dict = None):
        ...
        :param hourly_avgs: (optional) dict - keys datetime (the date to be updated), values: dict (hourly averages for the given date)
        """
        ...
        if len(self.cache_rawdata) > self.CACHE_LIMIT:
            ...
            for dat in dates[:len(dates)//2]: #since python 3 dictionary are ordered
                ...
                self.cache_rawdata.pop(dat, None)
                if hourly_avgs: self.cache_hourlyavgs.pop(dat, None)
        self.cache_rawdata.update(rawdata)
        if hourly_avgs: self.cache_hourlyavgs.update(hourly_avgs)
    ...
    def fetch_raw_data(self, dates : list) -> dict:
        ...
        """
        if type(dates) is not list or not all(isinstance(d, date) for d in dates):
            raise ValueError('Invalid dates')
        if self.sensor_table is not None and self.sensor_id is not None:
            #Fetch the raw data from database for the given sensor_id and the required concentrations for the given dates
            try:
                rawdata = (
                self.sensor_table.objects.values('obs_date', 'obs_time_utc', 'no2', 'particulatepm10', 'particulatepm2_5')
                ...
                rawdata = {'datetime': []}
                rawdata.update({c: [] for c in {'no2', 'particulatepm10', 'particulatepm2_5'}})
        else:
            #If sensor table is not set, return an empty dataframe
            rawdata = {'datetime': []}
            rawdata.update({c: [] for c in {'no2', 'particulatepm10', 'particulatepm2_5'}})
        #Convert the queryset to a pandas dataframe
        rawdata = pd.DataFrame(rawdata, columns=['datetime', 'no2', 'particulatepm10', 'particulatepm2_5'])
        rawdata.rename(columns={'particulatepm10': 'pm10', 'particulatepm2_5': 'pm2_5'}, inplace=True) #rename the columns to the required names
        ...
        for d in dates:
            data_by_date[d] = rawdata[rawdata.index.date == d]
        return data_by_date
